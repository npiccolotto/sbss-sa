...
KERNELS <- seq(50, 600, 50) %>% purrr::map(function(o) {return(c(0,o * 1000))})


setwd('/Users/npiccolotto/Projects/cvast/bssvis/meta-similarities-eval/snss-settings')
region1 <- get(load('1region.RData'))
cov3 <- get(load('cov3.RData'))
cov5 <- get(load('cov5.RData'))
diag_nw2se <- get(load('diag_nw2se.RData'))
diag_sw2ne <- get(load('diag_sw2ne.RData'))
grid_2x2 <- get(load('grid_2x2.RData'))
grid_2x2_botmerged <- get(load('grid_2x2_botmerged.RData'))
grid_2x2_topmerged <- get(load('grid_2x2_topmerged.RData'))
half_horizontal <- get(load('half_horizontal.RData'))
half_vertical <- get(load('half_vertical.RData'))
markus_guidance <- get(load('markus-guidance.RData'))
setwd('/Users/npiccolotto/Projects/cvast/bssvis/sbss-app/server/app/data/')
...
REGIONS <- list(
  ...
  half_vertical,
  markus_guidance
) %>% purrr::map(function(x) {return(x$regions)})
...
util_predict_to_grid <- function(S, P.spatial, x, y, p=2) {
  coords <- P.spatial@coords
  grid <- util_get_grid(coords, x, y)
  grid_data <- phylin::idw(S, coords, grid$grid, p)
...
space.k.geojson <-
  space.k %>% purrr::map(function(v) {
    ...
    circle <-
      c(1:24, 1) %>% purrr::map(function(t) {
        return(xy(center[1], center[2], v[2] - v[1], t / 24 * 2 * pi))
      }) %>% do.call(rbind , .)

    return(
      util_geojson_fc(
        list(
          util_polygon_to_geojson(spTransform(util_make_polygon(circle), WGS84)))
      )
    )
  })
...
points_in_same_region <- REGIONS %>% purrr::map(function(r) {
  vr <- util_regionalization_to_int_vector(r)
...
gmap <- ggmap::get_map(location=bbox, source='osm', zoom = 4, color = 'bw', force = TRUE)
...
p <- ncol(results[[1]]$s)
setwd(
  '/Users/npiccolotto/Projects/cvast/bssvis/meta-similarities/public/images/snss_gemas/w/'
)
...
json <- jsonlite::toJSON(list(
  ...
  rownames=list(
    k = params[,2] %>% purrr::map(function(k) {return(paste(round(KERNELS[[k]][2]/1000, digits = 2), 'km'))}),
    r = list()
  ),
  leafs = list(
...
write(x = json, file = 'snss_gemas.json')
